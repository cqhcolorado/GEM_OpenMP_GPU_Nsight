...
!$omp target data map(to:psi(:),mm(ns))
...
!$omp target data map(to:mm(ns),psi(:)) map(tofrom:mypfl_es(:),y3(:,ns),w2(:,ns),mypfl_em(:),mynos,myefl_em(:),myavewi,myefl_es(:),myke)
!$omp target teams loop private(rhox,rhoy)
#endif
  do m=1,mm(ns)
!     start_time_cpush = mpi_wtime()
...
!$omp target data map(to:mm(ns),w3e(1:mme),w111(1:mme),x3e(1:mme),z3e(1:mme),y3e(1:mme),u3e(1:mme),w011(1:mme),w001(1:mme),w101(1:mme),w000(1:mme),w100(1:mme),w010(1:mme),w110(1:mme)) map(tofrom:myden(:,:,:),mydene(:,:,:),myupar(:,:,:))
!$omp target teams loop private(rhox,rhoy)
#endif
     do m=1,mm(ns)
!        start_time_grid1 = mpi_wtime()
